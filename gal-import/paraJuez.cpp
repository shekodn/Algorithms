...
int main()
{
    ...
    galaxia gGalaxias[3000];
    //archivo.open("datos.txt");

    //Lee el archivo linea por linea y mete el código a un arreglo.
    while (cin>>iNumPlanetas)
    {
        gGalaxias[iCont].iNumeroPlanetas=iNumPlanetas;
        for(int i=0; i<iNumPlanetas; i++)
        {
            cin>>gGalaxias[iCont].pPlanetas[i].cId;
            cin>>gGalaxias[iCont].pPlanetas[i].fValor;
            cin>>gGalaxias[iCont].pPlanetas[i].sConexiones;
        }
        iCont++;
    }
    //archivo.close();
